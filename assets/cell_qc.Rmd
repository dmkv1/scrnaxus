...
```{r UMAP, fig.width=11, fig.height=5}
...
```{r discard_rates}
cluster_discard_stats <- table(colData(sce)[["seurat_clusters"]], colData(sce)[["discard"]])
...
cat("Removed", sum(colData(sce)$discard), "out of", ncol(sce), 
    "cells (", round(sum(colData(sce)$discard) / ncol(sce) * 100, 2), "%) marked for discard\n")
```
